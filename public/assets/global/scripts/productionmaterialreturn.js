...
$(function() {
	...
	$('#btn_save').on('click', function() {
		if ($('#returned_by').val() == '') {
			msg('Please fill out Returned By field.','failed');
		} 

		if ($('#date_returned').val() == '') {
			msg('Please fill out Date Returned field.', 'failed');
		} 

		if ($('#returned_by').val() !== '' && $('#date_returned').val() !== '') {
			openloading();
			var data = {
				_token: token,
				id: $('#ret_info_id').val(),
				...
				inv_id: $('input[name="details_inv_id[]"]').map(function () { return $(this).val(); }).get(),
	...
	$('#btn_save_details').on('click', function() {
		if ($('#actual_returned_qty').val() == '') {
			msg('Please provide amount of Actual Returned Quantity.', 'failed');
		} else {
	...
	$('#tbl_items_body').on('click', '.btn_pick_item', function() {
		...
		$('#required_qty').val($(this).attr('data-required_qty'));
		$('#return_qty').val($(this).attr('data-return_qty'));
        ...
        columns: [
	        ...
	        { data: function(x) {
				return x.issuanceno + '<input type="hidden" name="details_issuanceno[]" value="'+x.issuanceno+'">'+
						'<input type="hidden" name="details_inv_id[]" value="' + x.inv_id + '">' +
...
function makeInventoryTable(arr) {
	...
	$('#tbl_items').dataTable({
		...
		columns: [
			{
				data: function (x) {
					return "<button type='button' class='btn btn-sm btn-primary btn_pick_item' " +
								"data-inv_id='" + x.inv_id + "'"+
								"data-id='" + x.id + "'" +
								"data-required_qty='" + x.required_qty + "'" +
								"data-return_qty='" + x.return_qty + "'" +
								"data-item='" + x.item + "'" +
								"data-item_desc='" + x.item_desc + "'" +
								"data-lot_no='" + x.lot_no + "'" +
								"data-issued_qty='" + x.issued_qty + "'" +
								"data-issued_date='" + x.issued_date + "'>" +
